...
for i in glob('Data2/*')[:]:
    ...
    for path in glob('Data2/'+date+'/*.npy')[:]:
#         if path[:-3]+'P.txt' in glob('Data2/'+date+'/*.P.txt') and path[:-3]+'S.txt' in glob('Data2/'+date+'/*.S.txt'):
#             print('skip')
#             continue
        p_picks=[]
        s_picks=[]
        ...
        data=np.load(path)
        tt=np.arange(2,data.shape[1]/10+2,0.1)
        p_lib,s_lib,_,_=pick_picks(data[0],tt,0.5,0.6,10)
        for i in p_lib:
            p_picks.append([i,0,0])
        for i in s_lib:
            s_picks.append([i,0,0])
